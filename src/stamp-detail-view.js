...
function drawStampNote(r, stamp, left, top, width) {
  r.text('纪念短笺', left, top, 13, C.goldText);
  r.wrapped(STAMP_NOTES[stamp.id], left, top + 23, width, 14, C.ink, 21);
}

function drawStampProgress(r, stamp, left, top, width) {
  const right = left + width;
  r.line([[left, top], [right, top]], C.line, 1);
  r.text(stamp.owned ? '已收入邮票册' : '还差 ' + stamp.remaining + ' 星', left, top + 23, 18,
    stamp.owned ? C.green : C.goldText, 'left', '600');
  r.text('当前累计 ' + stamp.current + ' 星', left, top + 51, 12, C.muted);
  r.text(stamp.condition, right, top + 51, 12, C.muted, 'right');
  r.meter(left, top + 73, width, stamp.current, stamp.goal, stamp.owned ? C.green : C.gold);
}

function drawStampActions(r, game, stamp, replayLevels, left, top, width, close, heights) {
  const replayLevel = replayLevels[0];
  if (stamp.owned) {
    r.text('每关只计最高星级，达到条件后自动收藏。', 195, top, 13, C.muted, 'center');
    r.text('重走邮路，不会减少已收获的星星。', 195, top + 21, 13, C.muted, 'center');
    r.button('返回邮票册', left, top + 52, width, heights.primary, close, { style: 'primary', icon: 'stamp' });
    return;
  }
  r.text(replayLevel ? '已送达的来信中，还有 ' + replayLevels.length + ' 封可补星。' : '通关新关卡，星星会自动计入收藏。',
    195, top, 13, C.muted, 'center');
  r.text('三星需达目标拍数，且不使用道具或续灯。', 195, top + 21, 12, C.muted, 'center');
  r.button('去选关 · 继续旅程', left, top + 52, width, heights.primary,
    () => game.openLevelBrowser('all'), { style: 'primary', icon: 'route', size: 15 });
  if (replayLevel) r.button('重访 ' + String(replayLevel.id).padStart(3, '0') + ' · 补星', left, top + 60 + heights.primary, width, heights.secondary,
    () => game.openLevelBrowser('replay', replayLevel.id), { style: 'text', icon: 'restart', size: 14 });
}
...
function drawStampDetail(r, game, now) {
  ...
  // Reserve only real actions; an unavailable replay is not another button.
  const contentHeight = 346 + heights.primary + (!stamp.owned && replayLevels.length ? heights.secondary + 8 : 0);
  ...
  r.text('旅程纪念 · ' + filter.label + ' ' + (index + 1) + ' / ' + stamps.length, left, y + 57, 12, C.muted);
  r.button('关闭', right - 54, y + 17, 54, CONTROL.compactHeight, close, { style: 'text', size: 13 });
  ...
  drawStampNote(r, stamp, left, noteY, width);
  drawStampProgress(r, stamp, left, noteY + 78, width);
  drawStampActions(r, game, stamp, replayLevels, left, noteY + 176, width, close, heights);
  c.restore();
  return { x, y, w, h };
}
