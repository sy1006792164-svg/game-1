...
const { floorRing, risingLight, drawWindRibbon } = require('./spatial-effects');
...
function actorFrame(game, now, point, ghost = false, options = {}) {
  ...
  const idleTime = finite(options.idleTime) ? options.idleTime : time;
  const breathing = reduced ? 0 : Math.sin(idleTime / (ghost ? 390 : 610) + (ghost ? 1.7 : 0));
...
function burst(renderer, x, y, unit, progress, color, seed, paper, grounded = false, layer) {
  const c = renderer.ctx, low = renderer.effectsQuality === 'low';
  if (layer === 'ground' && low) return;
  ...
  for (let index = 0; index < count; index++) {
    ...
    const floorY = y + Math.sin(angle) * distance * TILT;
    const height = Math.sin(progress * Math.PI) * unit * (.24 + random(n + 4) * .42);
    const py = grounded ? floorY - height : y + Math.sin(angle) * distance * .6 - Math.sin(progress * Math.PI) * unit * .32 + (paper ? progress * progress * unit * .44 : -progress * unit * .12);
    const radius = 1.1 + random(n + 2) * 1.5;
    if (grounded && layer !== 'air' && !low && index % 2 === 0) {
      c.save(); c.globalAlpha *= .14 * (1 - progress);
      c.beginPath(); c.ellipse(px + height * .16, floorY + 1, radius * 1.5, radius * .5, 0, 0, TAU);
      c.fillStyle = '#315449'; c.fill(); c.restore();
    }
    if (layer === 'ground') continue;
    if (paper) {
      c.save(); c.translate(px, py); c.rotate(angle + progress * (random(n + 3) - .5) * 9);
      c.scale(1, .35 + Math.abs(Math.cos(angle + progress * 8)) * .65);
...
function drawEffectBatch(renderer, batch, now, point, scale, layer) {
  ...
  events.forEach((event, index) => {
    ...
    if (event.type === 'move') {
      ...
      if (ground) {
        floorRing(renderer, x, y + 2, scale * (.12 + progress * .23), COLORS.gold, 1.2);
      ...
      if (ground) floorRing(renderer, x, y + 1, scale * (.15 + progress * .48), color);
      if (air && !paper) risingLight(renderer, x, y, scale, progress, color);
      burst(renderer, x, y, scale, progress, color, seed, paper, true, layer);
      ...
      if (air) drawWindRibbon(renderer, [sx, sy], [x, y], scale, progress, COLORS.cyan);
    } else if (event.type === 'wait') {
      c.globalAlpha *= (1 - progress) * .75;
      if (ground) {
        floorRing(renderer, x, y, scale * (.13 + progress * .42), COLORS.cyan);
        if (!low) floorRing(renderer, x, y, scale * (.07 + progress * .29), COLORS.gold);
      }
    } else if (event.type === 'echo-born') {
      c.globalAlpha *= 1 - progress;
      if (ground) floorRing(renderer, x, y, scale * (.1 + progress * .45), COLORS.cyan);
      if (air) {
        risingLight(renderer, x, y, scale, progress, COLORS.cyan);
        renderer.icon('echo', x, y - scale * (.64 + progress * .15), scale * .25, COLORS.cyan);
      }
    } else if (event.type === 'undo') {
      c.globalAlpha *= 1 - progress;
      for (let ring = 0; ground && ring < (low ? 1 : 2); ring++) {
      ...
      if (air) renderer.icon('undo', x, y - scale * .65, scale * .28, COLORS.cyan);
      ...
      c.globalAlpha *= 1 - progress;
      if (ground) floorRing(renderer, x, y, scale * (.25 + progress * .75), color, won ? 2 : 1.3);
      if (!failed) {
        if (air) risingLight(renderer, x, y, scale * 1.4, progress, color);
        burst(renderer, x, y, scale * (won ? 1.5 : .85), progress, color, seed, true, true, layer);
      }
    }
    c.restore();
  });
...
function drawEffects(renderer, game, now, point, unit, options = {}) {
  ...
  if (options.layer !== 'air' && game.state && game.state.status === 'playing') drawBlocked(renderer, game, now, point, scale, reduced);
  if (reduced) return;
  buffer.batches.forEach(batch => drawEffectBatch(renderer, batch, now, point, scale, options.layer));
}
