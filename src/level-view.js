...
function drawLevelCard(r, game, level, record, index, rect, viewport, current) {
  ...
  r.panel(x, y, w, h, { fill: next ? '#284742' : unlocked ? C.panel : C.dark,
    stroke: next ? '#a18c5e' : C.line, accent: next ? C.gold : null, radius: 16 });
  ...
  if (record) {
    ...
    r.text(record.bestTurns + ' 拍', x + 146, y + h - 24, 10, C.muted, 'right');
  } else r.text(game.development ? '开发试玩' : unlocked ? '开始投递' : '先送达上一封', x + 17, y + h - 23, 11, next ? C.gold : unlocked ? C.green : C.muted);
  c.restore();
...
function drawLevels(r, game) {
  const profile = game.profile(), current = game.nextLevel(), scroll = game.levelScroll;
  ...
  r.button('回到进度', 250, 91, 116, 44, () => game.scrollToProgress(), 'quiet');
  if (game.development) r.button('输入关卡号', 130, 91, 114, 44, () => game.openDevelopmentPicker(), 'secondary');
  ...
  for (let chapter = first; chapter <= last; chapter++) {
    ...
    for (let index = chapter * PER_CHAPTER; index < end; index++) {
      ...
      if (rect.y + rect.h > viewport.y && rect.y < viewport.y + viewport.h) drawLevelCard(r, game, level, profile.completed[String(level.id)], index, rect, viewport, current);
    }
  }
