...
function assertFullSize(frame, width = 354) {
  const hero = frame.fills.find(item => item.color === C.green && item.w > 50);
  assert.ok(hero, 'the scorecard must remain painted during cached re-entry');
  assert.deepEqual(hero, { color: C.green, x: 18, y: 120, w: width, h: 126 }, 'the card occupies its full main-domain viewport');
