...
// Share repeated instructions in source while keeping the exported route data identical.
function serializeCampaign({ chapters, routes }) {
  const briefs = Array.from(new Set(routes.map(entry => entry[2])));
  const indices = new Map(briefs.map((text, index) => [text, index]));
  return "'use strict';\n\n// Generated by tools/generate.js. Maps are ordered by shortest route within each difficulty stage.\n// chapters: names for zero-based chapters 20 and later.\n// routes: [title, rows, brief, encoded witness].\nconst briefs = " +
    ...
    routes.map(entry => '    [' + entry.map((value, index) => index === 2 ? 'briefs[' + indices.get(value) + ']' : JSON.stringify(value)).join(',') + ']').join(',\n') +
    '\n  ]\n};\n';
}
...
function writeCampaign(messages, total) {
  ...
  const source = serializeCampaign({ chapters: chapterNames(chapterCount), routes: ordered.map(row => row.entry) });
...
module.exports = { tier, bridgesRequired, generate, search, draft, chapterWords, chapterNames, organizeRoutes, serializeCampaign, HAND_MADE, PER_CHAPTER, DEFAULT_TOTAL };
